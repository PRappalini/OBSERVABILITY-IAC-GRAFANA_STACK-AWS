...
1. **IPs Privadas Deterministas (`cidrhost`)**: En [terraform/ec2.tf](terraform/ec2.tf), las IPs privadas se calculan de forma estática en `locals`:
...
2. **Inyección Dinámica en Prometheus**:
   El script `bootstrap-prometheus.sh.tftpl` sustituye los placeholders `LOKI_PRIVATE_IP`, `TEMPO_PRIVATE_IP` y `GRAFANA_PRIVATE_IP` en [docker/prometheus/prometheus.yml](docker/prometheus/prometheus.yml) antes de levantar Docker Compose.
...
2. **Refactorización a Arquitectura Plana**:
   - Submódulos anidados reemplazados por archivos temáticos directos ([`network.tf`](terraform/network.tf), [`iam.tf`](terraform/iam.tf), [`security.tf`](terraform/security.tf), [`ec2.tf`](terraform/ec2.tf)).
...
5. **Sitio de Documentación Web con MkDocs & GitHub Pages**:
   ...
   - Pipeline de CI/CD automatizado en GitHub Actions ([`.github/workflows/deploy-docs.yml`](.github/workflows/deploy-docs.yml)) para publicación continua en GitHub Pages.
